...
{
    private const FRAMEWORKS = ['bootstrap3', 'bootstrap4', 'bootstrap5', 'tailwind'];
    ...
    public function handle(Filesystem $files)
    {
        ...
        if (!in_array($framework, self::FRAMEWORKS, true) || !in_array($theme, ['light', 'dark', 'system'], true)) {
            $this->error('Choose a supported framework and theme. No files were changed.');

            return 1;
        }
        if ($this->option('ui-kit') && !$this->installUiKit($framework)) {
            return 1;
        }

        $this->writeSettings($files, $framework, $theme);
    ...
    private function selectFramework()
    {
        ...
        if (!$framework && $this->input->isInteractive()) {
            $framework = $this->choice('Frontend framework', self::FRAMEWORKS, array_search($current, self::FRAMEWORKS));
        }

        return $framework ?: $current;
    }

    private function installUiKit($framework)
    {
        ...
        return $this->call('ui-kit:install', ['--css' => $framework, '--frontend' => 'blade']) === 0;
    }

    private function writeSettings(Filesystem $files, $framework, $theme)
    {
    ...
    private function publishViews(Filesystem $files)
    {
